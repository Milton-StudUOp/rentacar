        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 relative z-10">
                <div className="animate-fade-in-up">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 text-sm md:text-lg">{data?.meta?.total || 0} reservas registadas no sistema</p>
                </div>
            </div>

            {/* Advanced Filters Bar */}
            <div className="bg-white/90 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-200 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none backdrop-blur-md rounded-2xl p-4 md:p-5 border border-white/5 shadow-2xl flex flex-col md:flex-row gap-4 items-center z-20 relative animate-fade-in-up" style={{ animationDelay: '100ms' }}>

                {/* Left Side: Date Range */}
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                    <div className="relative w-full sm:w-auto z-50">
                        <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400 z-10 pointer-events-none" />
                        <Flatpickr
                            value={filterStartDate}
                            onChange={([date]) => updateUrlParams({ startDate: date ? format(date, 'yyyy-MM-dd') : null })}
                            options={{
                                locale: Portuguese,
                                dateFormat: 'd/m/Y',
                            }}
                            className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm font-medium text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-teal-500 transition-all hover:bg-slate-100 dark:hover:bg-slate-900 w-full sm:w-auto"
                            placeholder="Data de Início"
                        />
                    </div>
                    <span className="text-slate-500 dark:text-slate-400 text-sm hidden sm:block">até</span>
                    <div className="relative w-full sm:w-auto z-40">
                        <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400 z-10 pointer-events-none" />
                        <Flatpickr
                            value={filterEndDate}
                            onChange={([date]) => updateUrlParams({ endDate: date ? format(date, 'yyyy-MM-dd') : null })}
                            options={{
                                locale: Portuguese,
                                dateFormat: 'd/m/Y',
                        ...
                        {isStatusMenuOpen && (
                            <div className="absolute top-full left-0 mt-2 w-56 bg-white/90 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-md shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors duration-300 rounded-2xl border border-white/10 shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                                {[
                                    ...
                                    <label key={status.key} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-100 dark:bg-white/5 cursor-pointer transition-colors" onClick={(e) => { e.preventDefault(); toggleStatus(status.key); }}>
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedStatuses.includes(status.key) ? 'bg-cyan-500 border-cyan-500 text-slate-900' : 'border-slate-600 bg-slate-50 dark:bg-slate-900/50'
                                            }`}>
                                            {selectedStatuses.includes(status.key) && <CheckCircle className="w-3.5 h-3.5" />}
                                        </div>
                                        <span className="text-sm font-medium text-slate-600 dark:text-slate-300">{status.label}</span>
                                    </label>
                                ))}
                                {selectedStatuses.length > 0 && (
                                    <div className="px-4 py-2 border-t border-slate-200 dark:border-white/5 mt-1">
                                        <button
                                            onClick={() => updateUrlParams({ statuses: null })}
                                            className="w-full py-1.5 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors"
                                        >
                                            Limpar selecção
                                        </button>
                                    </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:flex-1 justify-end">
                    <div className="relative w-full sm:w-auto">
                        <ArrowDownUp className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400" />
                        <select
                            value={sortBy}
                        ...
                        >
                            ...
                            <option value="highest_value" className="bg-slate-900 text-slate-600 dark:text-slate-300">Maior Valor</option>
                            <option value="lowest_value" className="bg-slate-900 text-slate-600 dark:text-slate-300">Menor Valor</option>
                        </select>
                    </div>
                    ...
                    <div className="relative w-full sm:w-1/2 md:max-w-xs">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 border border-white/10 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all shadow-inner hover:bg-black/30"
                            id="search-bookings"
                        />
                    </div>
                </div>
            ...
            {/* Table */}
            <div className="bg-white/90 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-md shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors duration-300 rounded-2xl overflow-hidden mt-6 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                {isLoading ? (
                    ...
                    <>
                        {/* Desktop Table */}
                        <div className="overflow-x-auto pb-2 hidden lg:block">
                            <table className="w-full text-sm min-w-[900px] whitespace-nowrap">
                                <thead>
                                        ...
                                        return (
                                            <tr key={b.id} className="border-b border-slate-200 dark:border-white/5 hover:bg-white/[0.03] transition-all group relative">
                                                <td className="py-4 px-6 relative">
                                                    ...
                                                    <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 font-mono bg-slate-100 dark:bg-white/5 px-2 py-1 rounded-md border border-slate-200 dark:border-white/5 group-hover:border-cyan-500/30 group-hover:text-cyan-400 transition-colors">#{b.id}</span>
                                                </td>
                                                <td className="py-4 px-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            ...
                                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-mono mt-0.5">{b.user?.phone}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="py-4 px-4">
                                                ...
                                                <td className="py-4 px-4 text-right">
                                                    ...
                                                    <span className="text-[10px] text-slate-500 dark:text-slate-400 ml-1 font-medium">MT</span>
                                                </td>
                                                ...
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center justify-center gap-2 opacity-90 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        {b.status === 'PENDING' && (
                                                            <>
                                                                <button
                                                                    ...
                                                                    className="px-3 py-1.5 rounded-xl bg-emerald-500/10 text-emerald-400 text-xs hover:bg-emerald-500 border border-emerald-500/20 hover:text-slate-900 dark:text-white transition-all shadow-sm font-semibold flex items-center gap-1.5"
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800/50 hover:bg-red-500/10 text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-red-400 transition-all border border-slate-200 dark:border-white/5 hover:border-red-500/30 shadow-sm"
                                                        ...
                                                        {b.status === 'CONFIRMED' && (
                                                            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium italic bg-slate-100 dark:bg-white/5 px-2 py-1 rounded-md border border-slate-200 dark:border-white/5">Espera Pgto.</span>
                                                        )}
                                                        {b.status === 'PAID' && (
                                                        ...
                                                        {b.status === 'AWAITING_DELIVERY' && (
                                                            <button
                                                                ...
                                                                className="px-3 py-1.5 rounded-xl bg-purple-500/10 text-purple-400 text-xs hover:bg-purple-500 hover:text-slate-900 dark:text-white transition-all shadow-sm font-semibold border border-purple-500/30 flex items-center gap-1"
                                                            >
                                                                Entregar
                                                            </button>
                                                        )}
                                                        {b.status === 'DELIVERED' && (
                                                            <button
                                                                ...
                                                                className="px-3 py-1.5 rounded-xl bg-cyan-500/10 text-cyan-400 text-xs hover:bg-cyan-500 hover:text-slate-900 dark:text-white transition-all shadow-sm font-semibold border border-cyan-500/30 flex items-center gap-1"
                                                            >
                                                                Devolução
                                                            </button>
                                                        )}
                                    ...
                                    {filteredData?.length === 0 && (
                                        <tr>
                                            <td colSpan={8} className="py-20 text-center">
                                                <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center mx-auto mb-4 border border-white/10">
                                                    <Calendar className="w-8 h-8 text-slate-500 dark:text-slate-400" />
                                                </div>
                                                <p className="text-slate-600 dark:text-slate-300 font-bold text-lg">Nenhuma reserva localizada</p>
                                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">Tente ajustar os filtros de estado ou altere os seus termos de pesquisa para encontrar o que procura.</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                                ...
                                return (
                                    <div key={b.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-white/5 p-5 space-y-4 hover:border-cyan-500/30 transition-all flex flex-col relative overflow-hidden group shadow-sm">
                                        ...
                                        <div className="flex items-start gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 border border-cyan-500/20 flex items-center justify-center text-sm font-bold text-cyan-600 dark:text-cyan-400 shrink-0 shadow-lg group-hover:scale-110 transition-transform">
                                                {b.user?.name?.charAt(0)?.toUpperCase()}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between">
                                                    <p className="font-semibold text-sm truncate text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">{b.user?.name}</p>
                                        ...
                                        <div className="pt-3 border-t border-slate-200 dark:border-white/5 text-xs text-slate-500 dark:text-slate-400 flex justify-between items-center">
                                            ...
                                            <div className="text-right">
                                                <p className="font-bold text-teal-600 dark:text-teal-500 text-sm drop-shadow-sm">{formatPrice(b.totalPrice)} <span className="text-[10px]">MT</span></p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-3 border-t border-slate-200 dark:border-white/5 flex items-center justify-between gap-2 mt-auto">
                                            ...
                                            {b.status === 'PENDING' && (
                                                <div className="flex gap-2">
                                                    <button onClick={() => updateStatus.mutate({ id: b.id, status: 'CONFIRMED' })} className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-colors border border-emerald-500/20" title="Aprovar"><CheckCircle className="w-4 h-4" /></button>
                                                    <button onClick={() => updateStatus.mutate({ id: b.id, status: 'CANCELLED' })} className="w-8 h-8 rounded-lg bg-red-500/10 text-red-600 dark:text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors border border-red-500/20" title="Rejeitar"><XCircle className="w-4 h-4" /></button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    {[...Array(data.meta.totalPages)].map((_: unknown, i: number) => (
                        <button
                            ...
                            className={`w-9 h-9 rounded-xl text-sm font-medium transition-all ${page === i + 1
                                ...
                                : 'bg-white/90 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-md shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors duration-300 text-slate-300 hover:bg-white/5'
                                }`}
                        >
                            {i + 1}
                        </button>
            ...
            {selectedBooking && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200" onClick={() => setSelectedBooking(null)}>
                    <div className="bg-white/90 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-md shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors duration-300 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        {/* Background glow */}
                        <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/10 blur-[100px] rounded-full pointer-events-none"></div>

                        {/* Modal Header */}
                        <div className="px-8 py-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between sticky top-0 bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl z-20">
                            <h3 className="text-2xl font-black flex items-center gap-3">
                                <Eye className="w-6 h-6 text-cyan-400" />
                                Detalhes da Reserva <span className="text-slate-500 dark:text-slate-400 font-mono font-medium">#{selectedBooking.id}</span>
                            </h3>
                            <button onClick={() => setSelectedBooking(null)} className="p-2 rounded-xl text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-white/10 transition-colors">
                        ...
                        <div className="p-8 space-y-8 relative z-10">
                            ...
                            <div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-2xl p-6 space-y-4 hover:border-cyan-500/20 transition-colors">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500 to-cyan-400 flex items-center justify-center text-slate-900 dark:text-white font-bold">
                                            {selectedBooking.user?.name?.charAt(0)?.toUpperCase()}
                                        </div>
                                        <div>
                                            <p className="font-semibold">{selectedBooking.user?.name}</p>
                                            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Cliente #{selectedBooking.user?.id}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-2xl p-6 space-y-4 hover:border-cyan-500/20 transition-colors">
                                        ...
                                        <>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">{selectedBooking.vehicleBooking?.vehicle?.category} · {selectedBooking.vehicleBooking?.vehicle?.year}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
                                    ...
                                    {selectedBooking.notes && (
                                        <div className="pt-3 border-t border-slate-200 dark:border-white/5">
                                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Observações do cliente:</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-300 italic">"{selectedBooking.notes}"</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {selectedBooking.rating && (
                                <div>
                                    <h4 className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3 flex items-center gap-2">
                                    ...
                                    <div className="bg-gradient-to-br from-yellow-500/10 to-amber-500/5 border border-yellow-500/20 rounded-xl p-5 shadow-inner shadow-yellow-500/5 relative overflow-hidden">
                                        ...
                                        <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <span className="ml-3 text-sm font-bold text-yellow-400">
                                                {selectedBooking.rating}.0 <span className="text-slate-500 dark:text-slate-400 text-xs font-normal">/ 5.0</span>
                                            </span>
                                        </div>
                                        ...
                                        {selectedBooking.ratingComment ? (
                                            <div className="relative z-10">
                                                ...
                                                <p className="pl-6 text-sm text-slate-600 dark:text-slate-300 italic leading-relaxed">
                                                    "{selectedBooking.ratingComment}"
                                                </p>
                                            </div>
                                        ) : (
                                            <p className="text-sm text-slate-500 dark:text-slate-400 italic">O cliente não deixou nenhum comentário escrito.</p>
                                        )}
                                    </div>
                                </div>
                            )}
                            ...
                            {selectedBooking.status === 'PENDING' && (
                                <div className="flex gap-4 pt-4">
                                    <button
                                        ...
                                        className="flex-1 px-4 py-4 rounded-xl border border-white/10 hover:border-red-500/50 bg-slate-50 dark:bg-slate-900/50 hover:bg-red-500/10 text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-red-400 text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-sm"
                                    >
                            ...
                            {selectedBooking.status === 'CONFIRMED' && (
                                <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-blue-500/30 text-center flex flex-col items-center justify-center gap-3 shadow-lg">
                                    <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center">
                                        <Loader2 className="w-6 h-6 text-blue-400 animate-spin" />
                                    </div>
                                    <p className="text-slate-600 dark:text-slate-300 font-medium">Aguardando Pagamento do Cliente.</p>
                                </div>
                            )}
                                ...
                                <div className="flex flex-col gap-4 pt-4">
                                    {selectedBooking.payment?.receiptUrl && (
                                        <button
                                            ...
                                            className="w-full px-4 py-4 rounded-xl bg-white/90 dark:bg-slate-900/80 border border-cyan-500/30 text-cyan-400 text-sm font-bold hover:bg-cyan-500/10 hover:border-cyan-500/50 transition-all flex items-center justify-center gap-2 shadow-lg"
                                        >
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200" onClick={() => setViewingReceipt(null)}>
                    <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
                        <button
                            ...
                            className="absolute -top-12 right-0 md:-right-12 p-2 rounded-full text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors"
                        >
